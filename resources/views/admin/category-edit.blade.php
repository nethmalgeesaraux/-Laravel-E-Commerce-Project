<x-admin-layout>
    <main class="flex-1 p-6 overflow-y-auto">
        ...
        <div class="max-w-3xl mx-auto">
            <form action="{{ route('admin.category.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6 bg-white border border-gray-100 shadow-sm rounded-xl">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" id="name" name="name" placeholder="e.g. Electronics" value="{{ old('name', $category->name) }}" class="w-full px-4 py-2 border rounded-lg outline-none focus:ring-1 focus:ring-primary">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" id="slug" name="slug" placeholder="electronics" value="{{ old('slug', $category->slug) }}" class="w-full px-4 py-2 border rounded-lg outline-none bg-gray-50">
                        @error('slug')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex flex-col items-start gap-8 pt-4 md:flex-row">
                    <div class="w-full md:w-1/3">
                        ...
                        <div class="flex items-center justify-center w-full h-40 p-4 bg-white border rounded-lg">
                            <img src="{{ asset('uploads/categories/' . $category->image) }}" class="object-contain max-w-full max-h-full" alt="{{ $category->name }}">
                        </div>
                    </div>

                    <div class="w-full md:w-2/3">
                        <label class="block mb-2 text-sm font-medium text-gray-700">Change Category Image</label>
                        <div class="relative w-full h-40">
                            <label for="category-image" class="relative flex flex-col items-center justify-center w-full h-full overflow-hidden transition border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                                ...
                                <img id="image-preview" class="absolute inset-0 z-20 hidden object-contain w-full h-full p-2 bg-white" src="" alt="New Image Preview">
                                <input type="file" id="category-image" name="image" class="hidden" accept="image/png, image/jpeg, image/jpg, image/webp" />
                            </label>
                            <button type="button" id="remove-image-btn" class="absolute z-30 hidden w-8 h-8 text-red-500 transition-colors bg-white border border-gray-200 rounded-full shadow-md top-2 right-2 hover:text-white hover:bg-red-500" title="Remove new image">
